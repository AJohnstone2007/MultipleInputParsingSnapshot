...
public ARTTGParser(ARTGrammar artGrammar, ARTLexerV3 artLexer) {
  ...
  artBuildDirectives = "ARTDirectives [inputs=[], inputFilenames=[], directives={suppressPopGuard=false, tweLexicalisations=false, algorithmMode=gllGeneratorPool, tweLongest=false, tweSegments=false, termWrite=false, parserName=ARTTGParser, clusteredGSS=false, twePrint=false, inputPrint=false, lexerName=ARTTGLexer, trace=false, tweTokenWrite=false, tweDead=false, outputDirectory=., tweWrite=false, termPrint=false, suppressTestRepeat=false, twePriority=false, treeShow=false, tweRecursive=false, parseCounts=false, predictivePops=false, suppressProductionGuard=false, twePrintFull=false, tweFromSPPF=false, sppfPriority=false, tweExtents=false, sppfShow=false, tweSuffix=false, namespace=uk.ac.rhul.cs.csle.art.v3.tg, suppressSemantics=false, despatchMode=fragment, treePrintLevel=3, FIFODescriptors=false, twePrintSummary=false, treePrint=false, verbosity=0, gssShow=false, sppfLongest=false}]";
